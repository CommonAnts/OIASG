...
class Button(Control):
	...
	def on_resize(self):
		...
		if self.pressed_image is not None:
			self.pressed_image.update(x=self.x, y=self.y, scale_x=self.width / self.pressed_image.t_width,
			                          scale_y=self.height / self.pressed_image.t_height)
...
class TextBox(Control):
	...
	def __init__(self, control, text=None, style=None, editable=False, multiline=False, back=None, padding=0,
	             select_backcolor=None, select_textcolor=None, caret_color=None):
...
class FormattedTextBox(TextBox):
	def __init__(self, control, doc=None, editable=False, multiline=False, back=None, padding=0, select_backcolor=None,
	             select_textcolor=None, caret_color=None):
		super(TextBox, self).__init__(*control)
		self.editable = editable
		self.back = back
		self.padding = padding
	...
	def on_resize(self):
		...
		if self.bar is not None:
			...
			if self.direction == 0:
				...
				self.bar.update(x=R - (R - L) * ra / self.bar.t_width, y=D, scale_x=(R - L) * ra / self.bar.t_width,
				                scale_y=(U - D) / self.bar.t_height)
			elif self.direction == 2:
				self.bar.update(x=L, y=D, scale_x=(R - L) / self.bar.t_width, scale_y=(U - D) * ra / self.bar.t_height)
			elif self.direction == 3:
				self.bar.update(x=L, y=U - (U - D) * ra / self.height, scale_x=(R - L) / self.bar.t_width,
...
class Slider(Control):
	...
	def on_resize(self):
		...
		if self.image is not None:
			self.image.update(x=self.x, y=self.y + self.height / 2 - self.image.t_height / 2,
...
class ScrollBar(Slider):
	def on_resize(self):
		super(Slider, self).on_resize()
		if self.image is not None:
			self.image.update(x=self.x + self.width / 2 - self.image.t_width / 2, y=self.y,
			                  scale_y=self.height / self.image.t_height)
		if self.cursor is not None:
...
class ImageFrame(Frame):
	...
	def on_resize(self):
		...
		if self.front is not None:
			self.front.update(x=self.x, y=self.y, scale_x=self.width / self.front.t_width,
			                  scale_y=self.height / self.front.t_height)
	...
	def set_scrollbar(self, scrollbar):
		...
		if scrollbar is not None:
			...
			if self.doc is not None and self.doc.layout is not None:
				def f(event):
					def g(*args, **kw):
						self.doc.layout.ensure_line_visible(min(max(int(args[0] * self.doc.layout.get_line_count()), 0),
						                                        self.doc.layout.get_line_count() - 1))
...
class MessagePage(MessageBox):
	def __init__(self, control, back=None, front=None, title=Label(()), doc=TextBox((), '', {}), buttons=None,
	             layouter=MessagePage_defaultlayout):
