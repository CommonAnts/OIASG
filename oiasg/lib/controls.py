...
class Control(pyglet.event.EventDispatcher):
	...
	# width,height:控件大小
	# visible:是否在显示
	def __init__(self, window=None, pos=Posattr(), x=0, y=0, width=1, height=1, absx=None, absy=None, abswidth=None,
	             absheight=None, opacity=False):
...
class Button(Control):
	...
	def __init__(self, control, label=None, image=None, icon=None, pressed_image=None, direction=0,
	             color=(255, 255, 255), hover_color=(255, 255, 255), disabled_image=None):
...
class SwitchButton(Button):
	def __init__(self, control, labels=None, images=None, icons=None, pressed_images=None, direction=0,
	             color=(255, 255, 255), hover_color=(255, 255, 255), disabled_images=None):
...
class TextBox(Control):
	...
	# layout:文本
	# caret:光标(文本的)
	def __init__(self, control, text=None, style=None, editable=False, multiline=False, back=None, padding=0,
	             select_backcolor=None, select_textcolor=None, caret_color=None, valign='center'):
		...
		self.caret = None
		if self.editable:
			self.caret = Caret(self.layout, color=(
				caret_color if caret_color is not None else (style['color'][:3] if style.get('color') else (0, 0, 0))))
		self.on_resize()
	...
	def on_resize(self):
		if self.back is not None:
			self.back.update(x=self.x, y=self.y, scale_x=self.width / self.back.t_width,
			                 scale_y=self.height / self.back.t_height)
...
class FormattedTextBox(TextBox):
	def __init__(self, control, doc=None, editable=False, multiline=False, back=None, padding=0, select_backcolor=None,
	             select_textcolor=None, caret_color=None, valign='center'):
		super(TextBox, self).__init__(*control)
		self.editable = editable
		self.back = back
		self.padding = padding
...
class ProgressBar(Control):
	...
	def on_resize(self):
		...
		if self.bar is not None:
			K = self.bar_sizerate
			ra = self._rate
			L, R, U, D = self.x + self.width * K[0], self.x + self.width * (K[0] + K[2]), self.y + self.height * (
				K[1] + K[3]), self.y + self.height * K[1]
			if self.direction == 0:
				self.bar.update(x=L, y=D, scale_x=(R - L) * ra / self.bar.t_width, scale_y=(U - D) / self.bar.t_height)
			elif self.direction == 1:
				self.bar.update(x=int(R - (R - L) * ra / self.bar.t_width + 0.5), y=D,
				                scale_x=(R - L) * ra / self.bar.t_width, scale_y=(U - D) / self.bar.t_height)
			elif self.direction == 2:
				self.bar.update(x=L, y=D, scale_x=(R - L) / self.bar.t_width, scale_y=(U - D) * ra / self.bar.t_height)
			elif self.direction == 3:
				self.bar.update(x=L, y=int(U - (U - D) * ra / self.height + 0.5), scale_x=(R - L) / self.bar.t_width,
				                scale_y=(U - D) * ra / self.bar.t_height)
		self.resize_label(self.label)
...
class Slider(Control):
	...
	def on_resize(self):
		if self.image is not None:
			self.image.update(x=self.x, y=int(self.y + self.height / 2 - self.image.t_height / 2 + 0.5),
			                  scale_x=self.width / self.image.t_width)
...
class ScrollBar(Slider):
	def on_resize(self):
		if self.image is not None:
			self.image.update(x=int(self.x + self.width / 2 - self.image.t_width / 2 + 0.5), y=self.y,
			                  scale_y=self.height / self.image.t_height)
		if self.cursor is not None:
...
class Viewport(Frame):
	...
	def draw(self, range=None):
		...
		f = pyglet.image.get_buffer_manager().get_color_buffer().get_region(self.x, self.y, self.width,
		                                                                    self.height).get_texture()
		t.blit(0, 0)
...
class LRButtons(Grid):
	def __init__(self, control, buttons=None):
		super(LRButtons, self).__init__(control,
		                                layouter=Grid_ratelayout_gen(ROWS=1, COLUMNS=2, PADDING=0, ITEM_BLANKING=0))
...
class UDButtons(Grid):
	def __init__(self, control, buttons=None):
		super(UDButtons, self).__init__(control,
		                                layouter=Grid_ratelayout_gen(ROWS=2, COLUMNS=1, PADDING=0, ITEM_BLANKING=0))
		self.sons = buttons if buttons is not None else []
		self.relayout()
...
class ScrollTextBox(LayoutFrame):
	...
	def set_scrollbar(self, scrollbar):
		...
		if scrollbar is not None:
			...
			if self.doc is not None and self.doc.layout is not None:
				@scrollbar.event
				def on_change(rate):
					self.doc.layout.ensure_line_visible(
						min(max(int(rate * self.doc.layout.get_line_count()), 0), self.doc.layout.get_line_count() - 1))
		self.sons = list(filter(self.iskey, self.sons))
		self.relayout()
		self.check_scroll()
	...
	def check_scroll(self):
		...
		if self.doc.layout.get_point_from_line(self.doc.layout.get_line_count() - 1)[
			1] >= 0 and self.scrollbar.rate == 0:
...
def AlertBox_defaultlayout_gen(BUTTON_HEIGHT=30, BUTTON_BLANKING=10, PADDING=20, TITLE_PADDING=20, TITLE_HEIGHT=30,
                               BUTTON_X=(0.2, 20), BUTTON_WIDTH=(0.6, -40)):
	def f(self):
		...
		if self.doc is not None:
			self.doc.pos = Posattr((0, PADDING), (0, cur_h), (1, -2 * PADDING),
			                       (1, -cur_h - TITLE_PADDING - TITLE_HEIGHT))
...
class AlertBox(MessageInteractor, LayoutFrame):
	def __init__(self, control, back=None, front=None, title=Label(()), doc=None, button=None,
	             layouter=AlertBox_defaultlayout):
...
class MessageBox(MessageInteractor, LayoutFrame):
	def __init__(self, control, back=None, front=None, title=None, doc=None, buttons=None,
	             layouter=MessageBox_defaultlayout):
...
class Form(pyglet.window.Window):
	def __init__(self, width=None, height=None, caption=None, resizable=False, style=None, fullscreen=False,
	             visible=True, vsync=True, display=None, screen=None, config=None, context=None, mode=None):
		super(Form, self).__init__(width, height, caption, resizable, style, fullscreen, visible, vsync, display,
