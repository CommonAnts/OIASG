	...
	def init(self):
		...
		class Normal_ButtonLabel(pyglet.text.Label):
			def __init__(self, text):
				super().__init__(text, font_name=NORMAL_BUTTON_FONT, font_size=NORMAL_BUTTON_FONT_SIZE,
				                 color=NORMAL_BUTTON_FONT_COLOR, anchor_x='center', anchor_y='center')

		class Normal_ValueLabel(pyglet.text.Label):
			def __init__(self, text):
				super().__init__(text, font_name=NORMAL_VALUE_FONT, font_size=NORMAL_VALUE_FONT_SIZE,
				                 color=NORMAL_VALUE_FONT_COLOR, anchor_x='center', anchor_y='center')

		class Normal_Title0(Label):
			def __init__(self, text, pos=DEFAULT_POS):
				super().__init__((window, Posattr(*pos)), pyglet.text.Label(text, font_name=NORMAL_TITLE0_FONT,
				                                                            font_size=NORMAL_TITLE0_FONT_SIZE,
				                                                            color=NORMAL_TITLE0_COLOR,
				                                                            anchor_x='center', anchor_y='center'))

		class Normal_Title(Label):
			def __init__(self, text, pos=DEFAULT_POS):
				super().__init__((window, Posattr(*pos)),
				                 pyglet.text.Label(text, font_name=NORMAL_TITLE_FONT, font_size=NORMAL_TITLE_FONT_SIZE,
				                                   color=NORMAL_TITLE_COLOR, anchor_x='center', anchor_y='center'))

		class Normal_Title2(Label):
			def __init__(self, text, pos=DEFAULT_POS):
				super().__init__((window, Posattr(*pos)), pyglet.text.Label(text, font_name=NORMAL_TITLE2_FONT,
				                                                            font_size=NORMAL_TITLE2_FONT_SIZE,
				                                                            color=NORMAL_TITLE2_COLOR,
				                                                            anchor_x='center', anchor_y='center'))

		class Normal_Label(Label):
			def __init__(self, text, pos=DEFAULT_POS, anchor_x='left'):
				super().__init__((window, Posattr(*pos)),
				                 pyglet.text.Label(text, font_name=NORMAL_LABEL_FONT, font_size=NORMAL_LABEL_FONT_SIZE,
				                                   color=NORMAL_LABEL_COLOR, anchor_x=anchor_x, anchor_y='center'))
		...
		class Normal_TextInput(TextBox):
			def __init__(self, text, pos=DEFAULT_POS):
				super().__init__((window, Posattr(*pos)), text, NORMAL_INPUT_TEXT_STYLE, editable=True, multiline=False,
				                 select_backcolor=NORMAL_INPUT_TEXT_SELECT_BACKCOLOR,
				                 select_textcolor=NORMAL_INPUT_TEXT_SELECT_TEXTCOLOR,
				                 caret_color=NORMAL_INPUT_TEXT_SELECT_CARETCOLOR)
		...
		class Normal_ScrollBar(ScrollBar):
			def __init__(self, pos=DEFAULT_POS):
				super().__init__((window, Posattr(*pos)), image=Sprite(normal_scrollbar_back),
				                 cursor=Sprite(normal_scrollbar_cursor))
		...
		class Alert(AlertBox, UIControl):
			def __init__(self, text, title, pos=NORMAL_ALERT_POS):
				super().__init__((window, Posattr(*pos)), back=Sprite(normal_menu_back),
				                 layouter=AlertBox_defaultlayout_gen())
		...
		class Alert_Notitle(AlertBox, UIControl):
			def __init__(self, text, pos=NORMAL_ALERT_NOTITLE_POS):
				super().__init__((window, Posattr(*pos)), back=Sprite(normal_menu_back),
				                 layouter=AlertBox_defaultlayout_gen(TITLE_HEIGHT=0))
				alert_text = Normal_MessageText(text)
				self.doc = alert_text
				alert_button = Normal_Button(NORMAL_ALERT_BUTTON_TEXT)
				self.button = alert_button
		...
		class SettingMenu(MessageInteractor, UIControl):
			def __init__(self):
				...
				volume_effect_label = Normal_Label(SETTING_MENU_VOLUME_EFFECT_TEXT,
				                                   SETTING_MENU_VOLUME_EFFECT_LABEL_POS)
		...
		class ConfirmBox(MessageBox, UIControl):
			def __init__(self, text, pos=CONFIRM_BOX_POS):
				super().__init__((window, Posattr(*pos)), back=Sprite(normal_menu_back),
				                 layouter=MessageBox_defaultlayout_gen(TITLE_HEIGHT=0))
		...
		class VersionPage(AlertBox, UIControl):
			def __init__(self):
				...
				super().__init__((window, Posattr(*VERSION_PAGE_POS)), back=Sprite(normal_menu_back),
				                 layouter=AlertBox_defaultlayout_gen())
		...
		class AboutPage(AlertBox, UIControl):
			def __init__(self):
				super().__init__((window, Posattr(*ABOUT_PAGE_POS)),
				                 back=Sprite(normal_menu_back),
				                 layouter=AlertBox_defaultlayout_gen(TITLE_PADDING=50, PADDING=30, BUTTON_X=(0.5, -95),
				                                                     BUTTON_WIDTH=(0, 190)))
		...
		class AchievementPage_UIData(UIData_DictItem):
			def build_control(self):
				return Normal_Tag_rv(self.name), ImageFrame((window, Posattr(*self.pos)),
				                                            back=Sprite(resource.load_image(self.background)))
		...
		class AchievementMessage(MessageInteractor, UIControl):
			def __init__(self, item, got):
				...
				label = Normal_Title2(ACHIEVEMENT_MESSAGE_GOTTEN_TITLE if got else ACHIEVEMENT_MESSAGE_UNGOTTEN_TITLE,
				                      ACHIEVEMENT_MESSAGE_STATELABEL_POS)
		...
		class AchievementPage(MessageInteractor, UIControl):
			def __init__(self):
				...
				achievement_tagpages = TagPages([window],
				                                layouter=TagPages_defaultlayoutV_gen(TAG_HEIGHT=ACHIEVEMENT_TAG_HEIGHT,
				                                                                     TAG_PADDING=ACHIEVEMENT_TAG_PADDING,
				                                                                     TAG_WIDTH=ACHIEVEMENT_TAG_WIDTH,
				                                                                     FULL_PAGE=True))
		...
		class AppendicePage(MessageInteractor, UIControl):
			def __init__(self):
				...
				select_list_buttons = SelectButtons([window], layouter=Grid_defaultlayout_gen(
					ITEM_HEIGHT=APPENDICE_PAGE_SELECT_ITEM_HEIGHT, ITEM_BLANKING=APPENDICE_PAGE_SELECT_ITEM_BLANKING,
					PADDING=0))
		...
		class ScenarioPage(MessageInteractor, UIControl):
			...
			def __init__(self):
				...
				self.select_scroll = select_scroll
				select_list_buttons = SelectButtons([window], layouter=Grid_defaultlayout_gen(
					ITEM_HEIGHT=SCENARIO_PAGE_SELECT_ITEM_HEIGHT, ITEM_BLANKING=SCENARIO_PAGE_SELECT_ITEM_BLANKING,
					PADDING=0))
				self.select_list_buttons = select_list_buttons
				select_list.frame = select_list_buttons
				select_list.scrollbar = select_scroll
		...
		class MainMenu(ImageFrame, UIControl):
			def __init__(self):
				...
				start_button = Button(
					...
					label=pyglet.text.Label(MAIN_MENU_START_TEXT, font_name=MAIN_MENU_START_FONT,
					                        font_size=MAIN_MENU_START_FONT_SIZE, color=MAIN_MENU_START_COLOR,
					                        anchor_x='center', anchor_y='center'),
				...
				button_gen = lambda pos, text: Button(
					(window, Posattr(*pos)),
					label=pyglet.text.Label(text, font_name=MAIN_MENU_BUTTON_FONT, font_size=MAIN_MENU_BUTTON_FONT_SIZE,
					                        color=MAIN_MENU_BUTTON_COLOR, anchor_x='center', anchor_y='center')
				)
		...
		class SavesBox(ScrollFrame):
			...
			def __init__(self, pos=DEFAULT_POS):
				...
				select_list_buttons = SelectButtons([window], layouter=Grid_defaultlayout_gen(
					ITEM_HEIGHT=SAVESBOX_SELECT_ITEM_HEIGHT, ITEM_BLANKING=SAVESBOX_SELECT_ITEM_BLANKING, PADDING=0))
		...
		class GamePageMenu(MessageInteractor, UIControl):
			def __init__(self, gamepage):
				...
				buttons.sons = (
					back_button, save_button, achievement_button, appendice_button, setting_button, quit_button)
				buttons.relayout()
			...
			def __init__(self, gamedata, load_from_save=False):
				...
				class GamePageHeader(ImageFrame):
					def __init__(self):
						...
						class HeaderSwitchButton(SwitchButton):
							def __init__(self, pos=DEFAULT_POS):
								super().__init__((window, Posattr(*pos)), icons=[Sprite(game_header_switch_up_icon),
								                                                 Sprite(game_header_switch_down_icon)],
								                 disabled_images=[Sprite(game_header_switch_disabled_image)])
						...
						class SpeedBar(Frame):
							def __init__(self):
								...
								slider = ButtonSlider([window], layouter=ButtonSlider_defaultlayout_gen(PADDING_RATE=0,
								                                                                        HPADDING_RATE=0,
								                                                                        VPADDING_RATE=0))  # ,back = Sprite(game_header_speedbar_back))
								self.slider = slider
								self.sons.append(slider)

								class SliderButton(SwitchButton):
									def __init__(self):
										super(SliderButton, self).__init__([window], images=[
											Sprite(game_header_speedbar_button_back),
											Sprite(game_header_speedbar_pressed_back)])
								...
								label = Label((window, Middle_Posattr()),
								              pyglet.text.Label('', font_name=GAME_HEADER_SPEEDBAR_FONT,
								                                font_size=GAME_HEADER_SPEEDBAR_FONT_SIZE,
								                                color=GAME_HEADER_SPEEDBAR_FONT_COLOR,
								                                anchor_x='center', anchor_y='center'))
								self.label = label
							...
							def refresh(self):
								...
								self.label.text = (
									                  GAME_HEADER_SPEEDBAR_PAUSE_TEXT if gameplay.speed == 0 else '') + gameplay.ui_data.time.strftime(
									GAME_HEADER_SPEEDBAR_TIME_FORMAT)
						...
						class SwitchPageButtons(SelectButtons):
							def __init__(self):
								super().__init__((window, Posattr(*GAME_HEADER_SWITCH_PAGE_POS)),
								                 layouter=Grid_ratelayout_gen(COLUMNS=4, ROWS=1, PADDING=0,
								                                              ITEM_BLANKING=0))
								...
								class SwitchPageButton(SwitchButton):
									def __init__(self, icon_image, disabled_image=None):
										...
										super().__init__([window], icons=[icon, icon], images=[None, selected_image],
										                 disabled_images=None if disabled_image is None else [
											                 disabled_image, disabled_image], direction=4)

								self.buttons = [SwitchPageButton(game_header_switchpage_icons[i],
								                                 game_header_switchpage_disabled_icons[i]) for i in
								                range(GAME_HEADER_STATUS_SWITCHPAGE_COUNT)]
						...
						class StatusIconBars(Grid):
							def __init__(self, pos):
								...
								self.sons = [
									StatusIconBar(game_header_status_bar_icons[i], GAME_HEADER_STATUS_BAR_COLORS[i]) for
									i in range(GAME_HEADER_STATUS_BAR_COUNT)]
								self.relayout()
				...
				class GamePage_StrategyPlanBox(ScrollFrame):
					...
					def __init__(self, pos=DEFAULT_POS):
						...
						select_list_buttons = SelectButtons([window], layouter=Grid_defaultlayout_gen(
							ITEM_HEIGHT=GAME_SLPLAN_SELECT_ITEM_HEIGHT, ITEM_BLANKING=GAME_SLPLAN_SELECT_ITEM_BLANKING,
							PADDING=0), canceling=True)
						self.frame = select_list_buttons
					...
					def __init__(self):
						...
						class GamePages_Timetable_PlanFrame_Button(Button):
							def __init__(self, icon_image):
								super().__init__([window], image=Sprite(game_slplan_button_back),
								                 icon=Sprite(icon_image),
								                 pressed_image=Sprite(game_slplan_button_pressed_back), direction=4)

						buttons_grid = Grid((window, Posattr(*GAME_SLPLAN_BUTTONS_POS)),
						                    layouter=Grid_ratelayout_gen(ROWS=1, COLUMNS=4, PADDING=0, ITEM_BLANKING=0))
						...
						confirm_button = Button((window, Posattr(*GAME_SLPLAN_CONFIRM_POS)),
						                        image=Sprite(game_slplan_confirm_image))
						self.sons.append(confirm_button)
							...
							def __init__(self):
								...
								class GamePages_Timetable_MessageFrame(ImageFrame):
									def __init__(self, title_text='', pos=DEFAULT_POS):
										super().__init__((window, Posattr(*pos)),
										                 back=Sprite(game_pages_timetable_frame_back))
										...
										left_button = Button(
											(window, Posattr(*GAME_PAGES_TIMETABLE_FRAME_LEFT_BUTTON_POS)),
											image=Sprite(game_pages_timetable_frame_left_button_image))
										self.left_button = left_button
										self.sons.append(left_button)

										right_button = Button(
											(window, Posattr(*GAME_PAGES_TIMETABLE_FRAME_RIGHT_BUTTON_POS)),
											image=Sprite(game_pages_timetable_frame_right_button_image))
										self.right_button = right_button
										self.sons.append(right_button)

										item_grid = Grid((window, Posattr(*GAME_PAGES_TIMETABLE_FRAME_BOARD_POS)),
										                 layouter=Grid_defaultlayout_gen(PADDING=0, ITEM_BLANKING=0,
										                                                 ITEM_HEIGHT=GAME_PAGES_TIMETABLE_MESSAGES_ITEM_HEIGHT,
										                                                 CAL_ABSH=False))
										self.item_grid = item_grid
										self.sons.append(item_grid)

								timetable = GamePages_Timetable_MessageFrame(GAME_PAGES_TIMETABLE_TIMETABLE_TEXT,
								                                             GAME_PAGES_TIMETABLE_TIMETABLE_POS)
								self.timetable = timetable

								log = GamePages_Timetable_MessageFrame(GAME_PAGES_TIMETABLE_LOG_TEXT,
								                                       GAME_PAGES_TIMETABLE_LOG_POS)
								self.log = log
								...
								class GamePages_Timetable_ScrollStrategyFrame(ImageFrame):
									def __init__(self, title_text='', pos=DEFAULT_POS):
										...
										item_frame = SelectButtons([window], layouter=Grid_defaultlayout_gen(PADDING=0,
										                                                                     ITEM_BLANKING=0,
										                                                                     ITEM_HEIGHT=GAME_PAGES_TIMETABLE_MESSAGES_STRATEGY_ITEM_HEIGHT),
										                           canceling=True)
										# 越级访问
										...
										scroll_frame = ScrollFrame(
											(window, Posattr(*GAME_PAGES_TIMETABLE_FRAME_BOARD_POS)))
								...
								class GamePages_Timetable_PlanFrame(GamePages_Timetable_ScrollStrategyFrame):
									def __init__(self, title_text='', pos=DEFAULT_POS):
										...
										buttons_grid = Grid((window, Posattr(*GAME_PAGES_TIMETABLE_PLAN_BUTTONS_POS)),
										                    layouter=Grid_ratelayout_gen(ROWS=1, COLUMNS=6, PADDING=0,
										                                                 ITEM_BLANKING=0))
										self.buttons_grid = buttons_grid
										self.sons.append(buttons_grid)
										...
										class GamePages_Timetable_PlanFrame_Button(Button):
											def __init__(self, icon_image):
												super().__init__([window], image=Sprite(
													game_pages_timetable_frame_plan_button_back),
												                 icon=Sprite(icon_image), pressed_image=Sprite(
														game_pages_timetable_frame_plan_button_pressed_back),
												                 direction=4)

										add_button = GamePages_Timetable_PlanFrame_Button(
											game_pages_timetable_plan_add_icon)
										edit_button = GamePages_Timetable_PlanFrame_Button(
											game_pages_timetable_plan_edit_icon)
										remove_button = GamePages_Timetable_PlanFrame_Button(
											game_pages_timetable_plan_remove_icon)
										save_button = GamePages_Timetable_PlanFrame_Button(
											game_pages_timetable_plan_save_icon)
										# load_button = GamePages_Timetable_PlanFrame_Button(game_pages_timetable_plan_load_icon)
										moveup_button = GamePages_Timetable_PlanFrame_Button(
											game_pages_timetable_plan_moveup_icon)
										movedown_button = GamePages_Timetable_PlanFrame_Button(
											game_pages_timetable_plan_movedown_icon)
										shiftup_button = GamePages_Timetable_PlanFrame_Button(
											game_pages_timetable_plan_shiftup_icon)
										shiftdown_button = GamePages_Timetable_PlanFrame_Button(
											game_pages_timetable_plan_shiftdown_icon)
										...
										self.__dict__.update(
											{key: value for key, value in locals().items() if key[-7:] == '_button'})
										buttons_grid.sons = (
											add_button, edit_button, remove_button, save_button, move_grid, shift_grid)
										buttons_grid.relayout()

										strategy_cursor = SpriteControl([window],
										                                image=Sprite(game_message_strategy_cursor),
										                                fixed_ratio=False)
								...
								plan = GamePages_Timetable_PlanFrame(GAME_PAGES_TIMETABLE_PLAN_TEXT,
								                                     GAME_PAGES_TIMETABLE_PLAN_POS)
								self.plan = plan
								self.sons.append(plan)

								strategy = GamePages_Timetable_ScrollStrategyFrame(GAME_PAGES_TIMETABLE_STRATEGY_TEXT,
								                                                   GAME_PAGES_TIMETABLE_STRATEGY_POS)
				...
				class GameMessagesBoard(ImageFrame):
					def __init__(self):
						...
						timetable_label = Normal_Label(GAME_MESSAGES_BOARD_TIMETABLE_TEXT,
						                               GAME_MESSAGES_BOARD_TIMETABLE_LABEL_POS, anchor_x='center')
						self.timetable_label = timetable_label
						self.sons.append(timetable_label)

						timetable = Grid((window, Posattr(*GAME_MESSAGES_BOARD_TIMETABLE_POS)),
						                 layouter=Grid_defaultlayout_gen(PADDING=0, ITEM_BLANKING=0,
						                                                 ITEM_HEIGHT=GAME_MESSAGES_BOARD_TIMETABLE_ITEM_HEIGHT,
						                                                 CAL_ABSH=False))
						self.timetable = timetable
						self.sons.append(timetable)

						log_label = Normal_Label(GAME_MESSAGES_BOARD_LOG_TEXT, GAME_MESSAGES_BOARD_LOG_LABEL_POS,
						                         anchor_x='center')
						self.log_label = log_label
						self.sons.append(log_label)

						log = Grid((window, Posattr(*GAME_MESSAGES_BOARD_LOG_POS)),
						           layouter=Grid_defaultlayout_gen(PADDING=0, ITEM_BLANKING=0,
						                                           ITEM_HEIGHT=GAME_MESSAGES_BOARD_LOG_ITEM_HEIGHT,
						                                           CAL_ABSH=False))
						self.log = log
						self.sons.append(log)

						plan_label = Normal_Label(GAME_MESSAGES_BOARD_PLAN_TEXT, GAME_MESSAGES_BOARD_PLAN_LABEL_POS,
						                          anchor_x='center')
						self.plan_label = plan_label
						self.sons.append(plan_label)

						plan = Grid((window, Posattr(*GAME_MESSAGES_BOARD_PLAN_POS)),
						            layouter=Grid_defaultlayout_gen(PADDING=0, ITEM_BLANKING=0,
						                                            ITEM_HEIGHT=GAME_MESSAGES_BOARD_PLAN_ITEM_HEIGHT,
						                                            CAL_ABSH=False))
						self.plan = plan
						self.sons.append(plan)

						strategy_label = Normal_Label(GAME_MESSAGES_BOARD_STRATEGY_TEXT,
						                              GAME_MESSAGES_BOARD_STRATEGY_LABEL_POS, anchor_x='center')
						self.strategy_label = strategy_label
						self.sons.append(strategy_label)

						strategy_buttons = SelectButtons([window],
						                                 layouter=Grid_defaultlayout_gen(PADDING=0, ITEM_BLANKING=0,
						                                                                 ITEM_HEIGHT=GAME_MESSAGES_BOARD_STRATEGY_ITEM_HEIGHT),
						                                 canceling=True)
						# 越级访问
						...
						strategy_cursor = SpriteControl([window], image=Sprite(game_message_strategy_cursor),
						                                fixed_ratio=False)
						strategy_cursor.opacity = True
						self.strategy_cursor = strategy_cursor
						self.sons.append(strategy_cursor)
				...
				class GameCharacterBoard(ImageFrame):
					def __init__(self):
						...
						info = Grid((window, Posattr(*GAME_CHARACTER_BOARD_INFO_POS)),
						            layouter=Grid_defaultlayout_gen(COLUMNS=1, PADDING=0, ITEM_BLANKING=0,
						                                            ITEM_HEIGHT=None))
						info.sons = [Normal_Label('', anchor_x='center') for i in
						             range(GAME_CHARACTER_BOARD_INFO_COUNT)]
						self.info = info
						self.sons.append(info)
