...
class MessageItem(object):
	def __init__(self, name='', time=_DEFAULT_TIME, timeformat='', text='', image=None, font_name=None,
	             font_color=(0, 0, 0, 255), pos=None):
		self.__dict__.update({key: value for key, value in locals().items() if key != 'self'})
...
class Contest(object):
	def __init__(self, name='', image=None, problems=None, start_time=_DEFAULT_TIME, end_time=_DEFAULT_TIME,
	             time_delta=_DEFAULT_TIMEDELTA, ranklist=None):
		if problems is None:
			problems = {}
		if ranklist is None:
			ranklist = lambda: {}
