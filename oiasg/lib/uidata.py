...
class Message_Log_UIData(UIData):
	def build_control(self):
		...
		button_label = pyglet.text.Label(button_text,
		                                 font_name=self.data.font_name if self.data.font_name is not None else ui.GAME_MESSAGE_FONT_NAME,
		                                 font_size=ui.GAME_MESSAGE_FONT_SIZE, color=self.data.font_color,
		...
		messagewindow = ui.ui.MessageWindow(self.data.text, self.data.name, Sprite(
			resource.load_image(self.data.image)) if self.data.image is not None else None, self.data.pos)
...
class Strategy_UIData(UIData):
	def build_control(self):
		button_label = pyglet.text.Label(self.data.name, font_name=ui.GAME_STRATEGY_FONT_NAME,
		                                 font_size=ui.GAME_STRATEGY_FONT_SIZE, color=ui.GAME_STRATEGY_FONT_COLOR,
		                                 anchor_x='center', anchor_y='center')
...
class GameUIDataManager(object):
	...
	def messages_board_ui(self):
		...
		return list(map(self.get_message_timetable_button,
		                self.game.gamedata['timetable_messages'][-ui.GAME_MESSAGES_BOARD_TIMETABLE_ITEM_COUNT:])), list(
			map(self.get_message_log_button, self.game.gamedata['log_messages'][
			                                 -ui.GAME_MESSAGES_BOARD_LOG_ITEM_COUNT:])), strategy_board_items, list(
			map(self.get_strategy_button, self.game.gamedata['select_strategies']))
